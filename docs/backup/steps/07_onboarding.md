...
Cała wiedza o produkcie, copywritingu, pytaniach do użytkownika i układzie paywalla znajduje się w plikach `IDEA.md` oraz `ONBOARDING_AND_PAYWALL.md`. Przeczytaj je uważnie, zanim zaczniesz pisać kod.
...
- Ekran z pytaniem o główny ból ma być pytaniem zamkniętym. Użytkownik wybiera jedną z gotowych opcji. Możesz dodać opcję `Inne`, jeśli wynika to z `ONBOARDING_AND_PAYWALL.md`.
...
- Pytania i opcje odpowiedzi pobierz z `ONBOARDING_AND_PAYWALL.md`.

## 3. Fake Loading Screen (BARDZO WAŻNE)
- Ekran wyświetla loader (np. CircularProgressIndicator) i rotujące teksty (zmieniające się co ok. 1-1.5 sekundy). Teksty pobierz z `ONBOARDING_AND_PAYWALL.md`.
...
- **PERSONALIZACJA:** Jeśli onboarding zebrał imię lub inne dane do personalizacji, wykorzystaj je dynamicznie w rotujących tekstach zgodnie z `ONBOARDING_AND_PAYWALL.md`.

## 4. Aha Moment & Hard Paywall
- Ekran, na który użytkownik trafia bezpośrednio po "Fake Loading Screen".
- Najpierw pokaż osobny ekran `Aha Moment` z "Ziemią Obiecaną" / spersonalizowanym wynikiem zdefiniowanym w `ONBOARDING_AND_PAYWALL.md`.
- Ekran `Aha Moment` ma wykorzystywać dane z onboardingu, zwłaszcza wybrany główny ból, a imię użytkownika tylko jeśli wynika to z `ONBOARDING_AND_PAYWALL.md`.
- Na ekranie `Aha Moment` umieść CTA typu `Dalej`, które prowadzi do osobnego ekranu paywalla.
- Na osobnym ekranie paywalla zaimplementuj `Tytuł`, `Subtitle` oraz listę rzeczy odblokowywanych przez płatność zgodnie z `ONBOARDING_AND_PAYWALL.md`.
...
- `Home` po odrzuceniu paywalla ma wykorzystywać dane zebrane w onboardingu, jeśli `ONBOARDING_AND_PAYWALL.md` przewiduje nimi zasilenie pierwszego sensownego stanu ekranu.
...
## Krok 4. Utwórz `ONBOARDING_AND_PAYWALL.md`

| Status | Item |
| --- | --- |
| ⬜ | `ONBOARDING_AND_PAYWALL.md` istnieje |
| ⬜ | `ONBOARDING_AND_PAYWALL.md` opisuje flow `Welcome -> Continue as guest -> Onboarding -> Hard Paywall` |
| ⬜ | `ONBOARDING_AND_PAYWALL.md` jasno zakłada brak triala na tym etapie |
| ⬜ | `ONBOARDING_AND_PAYWALL.md` opisuje pytania onboardingowe i ich cel |
| ⬜ | `ONBOARDING_AND_PAYWALL.md` zawiera zamknięte pytanie o główny ból użytkownika |
| ⬜ | `ONBOARDING_AND_PAYWALL.md` zawiera gotowe opcje odpowiedzi dla pytania o ból |
| ⬜ | `ONBOARDING_AND_PAYWALL.md` zawiera pytanie zbierające minimalne dane do pierwszego sensownego stanu `Home` |
| ⬜ | `ONBOARDING_AND_PAYWALL.md` opisuje, jak aplikacja komunikuje zrozumienie problemu użytkownika |
| ⬜ | `ONBOARDING_AND_PAYWALL.md` definiuje moment `Aha!` |
...
| ⬜ | `ONBOARDING_AND_PAYWALL.md` zawiera konkretne teksty dla ekranu analizy / ładowania |
| ⬜ | `ONBOARDING_AND_PAYWALL.md` zawiera konkretne dane paywalla, a nie tylko ogólne hasła |
| ⬜ | `ONBOARDING_AND_PAYWALL.md` zawiera `Tytuł`, `Subtitle` i listę rzeczy odblokowywanych przez płatność |
| ⬜ | Kluczowe sekcje `ONBOARDING_AND_PAYWALL.md` zostały uzupełnione treścią, a nie zostawione jako puste hasła dla usera |




4. Przygotuj `ONBOARDING_AND_PAYWALL.md` w głównym katalogu projektu.

Ten dokument ma wynikać z ustaleń zapisanych wcześniej w `IDEA.md`.

`ONBOARDING_AND_PAYWALL.md` jest dokumentem pierwszej sesji i konwersji.
Ma opisywać:
...
Nie każ userowi samodzielnie wymyślać treści tych sekcji. To Ty masz je zaproponować i uzupełnić na podstawie rozmowy z kroku 2 oraz na podstawie `IDEA.md`.
